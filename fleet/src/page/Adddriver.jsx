...
const Adddriver = () => {
  ...
  async function submit() {
    if (ifile) {
      ...
      const { error } = await supabase
        .from("worker")
        .insert({ photo:zpath , name:nameref.current.value , vehiclename:vehicle , vehicleid:vehicleidref.current.value , type:'driver' , email:emailref.current.value, password:passwordref.current.value});
  ...
  async function signupfun(){
    try{
      await Driversignup(emailref.current.value,passwordref.current.value)
      navigate('/admin')
    }
    catch(err){
  ...
  return (
    <div className={styles.box}>
      <div className={styles.first} style={{display:show?'flex':'none'}}>
        <div className={styles.top}>Adddriver</div>
