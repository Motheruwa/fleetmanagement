  ...
  async function submit() {
    if (ifile) {
      ...
      const { error } = await supabase
        .from("worker")
        .insert({ photo:zpath , name:nameref.current.value , vehiclename:vehicle , vehicleid:vehicleidref.current.value , type:'fuel manager' , email:emailref.current.value, password:passwordref.current.value});
  ...
  return (
    <div className={styles.box}>
      <div className={styles.first} style={{display:show?'flex':'none'}}>
        <div className={styles.top}>Add fuel manager</div>
